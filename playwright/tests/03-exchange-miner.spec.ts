...
test.describe('Exchange Miner', () => {
    test.afterEach(async ({ sharedPage: page }) => {
        await stopCpuMiningDirect(page);
    });
    ...
    test('copy wallet address and switch to exchange mining', async ({ sharedPage: page }) => {
        ...
        // In exchange mode, WalletBalanceHidden renders with a distinct test ID
        const balanceEl = page.locator(sel.wallet.balanceExchange);
    ...
    test('switch back to Tari Universe wallet and verify balance', async ({ sharedPage: page }) => {
        // Read current balance before reverting so the assertion is self-contained
        const balanceBeforeRevert = await getWalletBalance(page);
        ...
        const normalBalance = page.locator(sel.wallet.balance);
        const start = Date.now();
        while (Date.now() - start < 180_000) {
            try {
        ...
        expect(balanceAfter).toBeGreaterThanOrEqual(balanceBeforeRevert);
    });
