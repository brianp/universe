import { initWebSocket, getWsReady, addEventCallback } from './tauri-remote-ui-socket';
...
export async function listen(event: string, handler: (data: unknown) => void, _options?: unknown): Promise<UnlistenFn> {
    if (
...
export async function once(event: string, handler: (data: unknown) => void, options?: unknown): Promise<UnlistenFn> {
