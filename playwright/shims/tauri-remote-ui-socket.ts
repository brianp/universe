...
function getWsReady(): Promise<void> | null {
    return wsReady;
}
...
function rejectAllInFlight(reason: string) {
    for (const id of Object.keys(filterCollection)) {
        try {
            filterCollection[Number(id)]({ status: 'error', payload: reason });
        } catch {}
...
function initWebSocket() {
    ...
    wsReady = new Promise<void>((resolve, reject) => {
        ...
        socket.onclose = (e) => {
            ...
            // Schedule reconnect — don't resolve, let callers retry via initWebSocket
...
export { filterCollection, initWebSocket, getWs, getWsReady, addEventCallback };
