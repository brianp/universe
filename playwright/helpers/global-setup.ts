...
import { TEST_WALLET, APP_ID, getAppConfigDir } from './test-wallet';
...
function getWalletConfig() {
    return {
        ...
        monero_address:
            '49aN3cwox5jCz9grdZmd5KTYggXrtVKRAdpr4wTba27HBxw4d29qfMj6rMNPNgAPhPgEyXEGWiNyZYLAKdPjn7CUPybeYYA',
        wxtm_addresses: {},
    ...
    if (skipBackend) {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            appProcess.on('exit', (code, signal) => {
                reject(new Error(`Tauri process exited during startup: code=${code} signal=${signal}`));
            });
            setTimeout(() => {
                // Process survived the startup window — remove the exit listener
                // so normal shutdown doesn't trigger a spurious rejection.
